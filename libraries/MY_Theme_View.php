...
class Theme_View extends Theme_View_Core {
  ...
  public function breadcrumb_menu($theme, $parents) {
    ...
    elseif ($this->item() and (!empty($parents) or (empty($parents) and $this->breadcrumbs_showinroot))):
      $content .= '<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12" style="width: auto; padding-left: 0;"><ul class="breadcrumb g-breadcrumbs g-' . $this->breadcrumbs_position . '">';
      $i = 0;
      if (!empty($parents)):
  ...
  public function get_thumb_element($item, $addcontext = FALSE, $linkonly = FALSE) {
    ...
    $content = '<li id="g-item-id-' . $item->id . '" class="g-item col-lg-fifth col-md-3 col-sm-4 col-xs-12 ' . $item_class . ' ' . $this->thumb_type;
    if ($item->is_album()):
